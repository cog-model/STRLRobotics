...
public:
            ...
            for(int n = 1; n <= extraCheck; n++)
            {
                ...
                for(k = 1; k <= num; k++)
                    if(map.CellOnGrid(x1 - n*step_x, y1 + k*step_y))
                        if(map.CellIsCloseToObstacle(x1 - n*step_x, y1 + k*step_y))
                            return false;
                for(k = 1; k <= num; k++)
                    if(map.CellOnGrid(x2 + n*step_x, y2 - k*step_y))
                        if(map.CellIsCloseToObstacle(x2 + n*step_x, y2 - k*step_y))
                            return false;
            }
            error = 0;
            for(x = x1; x != x2 + step_x; x+=step_x)
            {
                if(map.CellIsCloseToObstacle(x, y))
                    return false;
                if(x < x2 - extraCheck)
                {
                    num = (gap + error)/delta_x;
                    for(k = 1; k <= num; k++)
                        if(map.CellIsCloseToObstacle(x, y + k*step_y))
                            return false;
                }
                if(x > x1 + extraCheck)
                {
        ...
        else
        {
            ...
            for(int n = 1; n <= extraCheck; n++)
            {
                ...
                for(k = 1; k <= num; k++)
                    if(map.CellOnGrid(x1 + k*step_x, y1 - n*step_y))
                        if(map.CellIsCloseToObstacle(x1 + k*step_x, y1 - n*step_y))
                            return false;
                for(k = 1; k <= num; k++)
                    if(map.CellOnGrid(x2 - k*step_x, y2 + n*step_y))
                        if(map.CellIsCloseToObstacle(x2 - k*step_x, y2 + n*step_y))
                            return false;
            }
            error = 0;
            for(y = y1; y != y2 + step_y; y += step_y)
            {
                ...
                if(y < y2 - extraCheck)
                {
                    num = (gap + error)/delta_y;
                    for(k = 1; k <= num; k++)
                        if(map.CellIsCloseToObstacle(x + k*step_x, y))
                            return false;
                }
                if(y > y1 + extraCheck)
                {
    ...
    template <class T>
    bool getTwoClosestObstacles(int x, int y, const T &map, int &i1, int &j1, int &i2, int &j2)
    {
        i1 = -1; j1 = -1; i2 = -1; j2 = -1;
        for (int d = 1; d <= 1.5 * agentSize; d++)
        {
            if (i2 > 0)
            {
                break;
            ...
            for (int k = 0; k < cells.size(); k++)
            {
                ...
                if ((abs(cells[k].first) + abs(cells[k].second) == d) && (map.CellIsObstacle(x + cells[k].first, y + cells[k].second)))
                {
                    if (i1 < 0)
                    {
                        i1 = x + cells[k].first;
                    ...
                    else if (abs(i1 - (x + cells[k].first)) + abs(j1 - (y + cells[k].second)) > std::max(int(agentSize), abs(i1 - x) + abs(j1 - y)))
                    {
                        i2 = x + cells[k].first;
                        j2 = y + cells[k].second;
                        break;
