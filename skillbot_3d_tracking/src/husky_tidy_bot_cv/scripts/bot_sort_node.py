...
def build_parser():
    ...
    parser.add_argument('--node-name', type=str, default="tracking")
    parser.add_argument('--segmentation-topic', type=str)
    parser.add_argument('--image-topic', type=str)
    parser.add_argument('--out-tracking-topic', type=str)
...
class BoTSORT_node(BoTSORT_wrapper):
    ...
    def start(self):
        ...
        self.sync_sub = message_filters.TimeSynchronizer(
            [self.segmentation_sub, self.image_sub], 50)
...
def complete_args(args):
    ...
    if args.segmentation_topic is None:
        args.segmentation_topic = "/segmentation"
    ...
    if args.out_tracking_topic is None:
        args.out_tracking_topic = "/tracking"
    ...
    if args.enable_visualization and args.out_visualization_topic is None:
        args.out_visualization_topic = "/tracking_vis"

    assert args.segmentation_topic is not None
    assert args.image_topic is not None
    assert args.out_tracking_topic is not None
...
if __name__ == "__main__":
    ...
    tracking_node = BoTSORT_node(
        args.segmentation_topic,
        args.image_topic,

        args.out_tracking_topic,
