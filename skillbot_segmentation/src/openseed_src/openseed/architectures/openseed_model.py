...
class OpenSeeD(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.task_switch = {'coco': coco_on, 'o365': o365_on, 'rosbag': rosbag_on, 'lvis': lvis_on}
    ...
    def forward(self, batched_inputs, inference_task='seg'):
        ...
        if self.training:
            ...
            if self.task_switch['lvis']:
                #self.criterion.num_classes = 133 if 'pano' in self.train_dataset_name[0] else 80
                self.criterion.num_classes = 31
                task = 'seg'
                if not self.coco_mask_on:
                    task = 'det'
                # import ipdb; ipdb.set_trace()
    ...
    def forward_seg(self, batched_inputs, task='seg'):
        ...
        if self.training:
            ...
            for mask_cls_result, mask_pred_result, mask_box_result, input_per_image, image_size in zip(
                ...
                #print("Height", height)
                #print("Width", width)
                #print(self.semantic_on, self.panoptic_on, self.sem_seg_postprocess_before_inference, self.instance_on)
                #print((images.tensor.shape[-2], images.tensor.shape[-1]))
                ...
                if self.instance_on:
                    ...
                    #print(instance_r.pred_masks.shape)
                    #input()
