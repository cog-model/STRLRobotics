...
from openseed_model_onnx import OpenSeeD_wrapper_onnx
...
def build_parser():
    ...
    parser.add_argument('-onnx', '--onnx-model', action='store_true')
    return parser


class OpenSeeD_node:
    def __init__(self, categories, model_file, weights_file, image_topic, out_segmentation_topic, labels_topic=None, 
            out_visualization_topic=None, min_score_seen=0.7, min_score_unseen=0.1, palette=((0, 0, 255),), onnx=False,
            language_config=None, language_weights=None):

        if onnx:
            self.model = OpenSeeD_wrapper_onnx(
                model_file, language_config, language_weights,
                min_score_seen=min_score_seen,
                min_score_unseen=min_score_unseen
            )
        else:
            self.model = OpenSeeD_wrapper(
        ...
        with self.total_tm:
            ...
            with self.segm_tm:
                scores, classes_ids, boxes, masks = \
                    self.model.segment(image, self.categories,
...
if __name__ == "__main__":
    ...
    if args.onnx_model:
        ...
        "/home/administrator/zemskova_ts/husky_tidy_bot_cv_ws/src/openseed_src//openseed_aug.onnx",
        None,
        "/realsense_gripper/color/image_raw/compressed", "/segmentation", labels_topic="/segmentation_labels",
        out_visualization_topic=out_visualization_topic, min_score_seen=0.45, min_score_unseen=0.2, onnx=True,
        language_config="/home/administrator/zemskova_ts/husky_tidy_bot_cv_ws/src/openseed_src/openseed_swint_lang_rosbag.yaml",
        language_weights="/home/administrator/zemskova_ts/husky_tidy_bot_cv_ws/src/openseed_src/openseed_lang_encoder.pth",
        )
    else:
        segmentation_node = OpenSeeD_node(categories,
            "/home/administrator/zemskova_ts/husky_tidy_bot_cv_ws/src/openseed_src/openseed_swint_lang_rosbag.yaml",
            "/home/administrator/zemskova_ts/husky_tidy_bot_cv_ws/src/openseed_src/model_aug.pth",
    #        "/home/wingrune/cv/OpenSeeD/configs/openseed/openseed_swint_lang_rosbag.yaml",
    #        "/hdd/wingrune/openseed/output_finetune/model_0003999.pth",
            "/realsense_gripper/color/image_raw", "/segmentation", labels_topic="/segmentation_labels",
            out_visualization_topic=out_visualization_topic, min_score_seen=0.45, min_score_unseen=0.2)
    ...
    if len(segmentation_node.output_delays) > 0:
        logs_folder = osp.abspath(osp.expanduser(osp.join(
            osp.dirname(__file__), "logs/openseed_node")))
